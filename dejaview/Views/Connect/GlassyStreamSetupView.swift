...
struct GlassyStreamSetupView: View {
    ...
    var body: some View {
        GeometryReader { geometry in
            ...
            .safeAreaInset(edge: .bottom, spacing: 0) {
                let layout = usesHorizontalActions(in: geometry.size)
                ...
                .frame(maxWidth: usesHorizontalActions(in: geometry.size) ? 620 : 460)
                .padding(.horizontal, 24)
                .padding(.top, 12)
                .padding(.bottom, 12)
                .frame(maxWidth: .infinity)
    ...
    private func usesHorizontalActions(in size: CGSize) -> Bool {
        !dynamicTypeSize.isAccessibilitySize && size.width >= 560 && size.height < 400
    }
